...
class EvaluasiController extends Controller
{
    ...
    {
        foreach ($matrix as $m) {
            $result[$m->id_alternatif][$m->id_kriteria] = $m->value;
        }
    ...
    public function weightingNormalizedMatrix($nMatrix, $criterias)
    {
        for ($i = 1; $i <= count($nMatrix[1]); $i++) { // criteria
            for ($j = 1; $j <= count($nMatrix); $j++) { // alternative
                $v[$j][$i] = $nMatrix[$j][$i] * $criterias[$i - 1]->bobot;
            }
        }
    ...
    public function findConcordanceMatrix(array $c, $criterias, $m)
    {
        ...
        for ($k = 1; $k <= $m; $k++) {
            ...
            for ($l = 1; $l <= $m; $l++) {
                if ($k != $l && count($c[$k][$l])) {
                    foreach ($c[$k][$l] as $j) {
                        $C[$k][$l] = (isset($C[$k][$l]) ? $C[$k][$l] : 0) + $criterias[$j - 1]->bobot;
                    }
                }
            }
        }
